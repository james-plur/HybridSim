...
def resolved_kv_formula(model: ModelRef) -> str:
    ...
    if raw:
        return raw
    # Infer from attention variant when kv_formula unset.
...
def _num_indexer_layers(model: ModelConfig) -> int:
    ...
    # IndexShare: offset + floor((L - offset) / freq)
...
def cache_bytes(
    ...
    *,
    ...
    if formula == KV_FORMULA_V4_HYBRID:
        ...
        # Indexer: ratio=4 layers, floor(T/4) slots
        idx_elements = ratio4_layers * (tokens // 4) * idx_hd
        nbytes += idx_elements * idx_prec
        if draft:
            # calc.js: draft layers = ratio==0 count; adds their sliding window again

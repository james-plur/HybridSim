"""Export analytical Operator DAG workloads as Chrome Trace (torch-profiler style).

Schedules kernels with ASAP (earliest-ready) timing so compute/comm overlap is
visible when dependencies allow it. Multi-rank views replicate the same schedule
onto TP×PP logical ranks for inspection (analytical model is rank-symmetric).
"""
...
from hybridsim_infer.workload_generators.analytic_model.types import OperatorKind
...
def _tid_for_kernel(name: str, kind: Optional[str] = None) -> int:
    lower = (name or "").lower()
    if kind == OperatorKind.COMM.value or kind == "comm":
        return TID_COMM
    if any(tok in lower for tok in ("allreduce", "allgather", "reduce_scatter", "send_recv", "dispatch", "combine", "p2p")):
        return TID_COMM
    if any(tok in lower for tok in ("layernorm", "residual", "emb", "kv_cache_save")):
        return TID_MEMORY
    return TID_COMPUTE
...
def asap_schedule(kernels: list[dict[str, Any]]) -> list[ScheduledKernel]:
    ...
    for i, k in enumerate(kernels):
        ...
        ready = max((starts[d] + float(kernels[d].get("duration", 0.0)) for d in deps), default=0.0)
        starts[i] = ready
...
def _analyze_with_features(
    generator: OpWorkloadGenerator,
    batch: ScheduleBatch,
    *,
    workload_id: int = 1,
) -> list[dict[str, Any]]:
    """Like OpAnalyzer.analyze but keep flops/bytes/kind on each kernel."""
    ...
    for op_idx, op in enumerate(op_dag.operators):
        ...
        for local_i, plan in enumerate(plans):
            ...
            kind = (
                plan.kind.value
                if hasattr(plan.kind, "value")
                else str(getattr(op, "kind", "") or "")
            )
            kernels.append(
                {
                    "name": plan.name,
...
def profile_schedule_batch(
    batch: ScheduleBatch,
    *,
    analytical: AnalyticalConfig | None = None,
    ...
    gen = OpWorkloadGenerator(analytical=cfg)
...
# Re-export for callers that only need features alongside profile.
__all__ = [
    "ScheduledKernel",
    "asap_schedule",
    "build_chrome_trace",

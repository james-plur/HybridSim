...
| 本地 GPU 块 | `KVCacheManager` / BlockPool | `kv_system.VllmKvCacheManager` |
| Store 元数据池 | `mooncake_master` + Store connector | `kv_system.MooncakeKvStore` |
| Store worker CRUD 打点 | `mooncake/store/pool_profile.py` + `worker.py` | `_emit` / `pool_recorder` |
...
Case JSON 可用 `scheduler.store_num_blocks` 限制 offline 池容量（默认 4096），用于触发 LRU `evict`。

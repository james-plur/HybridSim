...
|------|-----------------|-----------|------|
| Predict | `fixed` / `token_proportional` / `predict` | `PredictWorkloadGenerator` | **1** 个 TimeoutKernel（整 batch 时长） |
| Op + analytical | `analytical`（别名 `analytic` / `op` / `kernel_dag`） | `OpWorkloadGenerator` | **多层 Operator DAG** → 多个 TimeoutKernel（Roofline / α-β） |

工厂入口：`workload_generators/factory.py`。

- **predict**：委托 Frontier `BaseExecutionTimePredictor`（通常 RandomForest）估 `ExecutionTime.total_time`。
- **op + analytical**：先按 `rf_catalog` 构造与 Frontier 算子名对齐的 DAG，再对每个 op 做解析计时；依赖正确时自然支持计算与通信交叠。后续也可把 op 交给底层 kernel（当前未做）。
...
2. `build_operator_dag` 按层串起与 Frontier 同名的物理算子（见 `rf_catalog.py`）。
3. `OpAnalyzer`：compute → Roofline；comm → α-β；乘以全局 `duration_scale`。
4. 串行时长用 critical-path（可交叠时比 sum 更短）。

### 3.1 结构对齐

`DENSE_ATTN_*` / `MLA_ATTN_*` / FFN / MoE / memory / TP-PP comm 名称与 Frontier OperatorSpec 对齐；`tests/test_op_workload_generator.py` 覆盖 DAG 名与依赖无环。
...
实测（2026-08-12，非 dummy RF + util=0.6）：
...
- **`model_preset`**：模型形状、注意力变体、KV 字节公式的公用真源（analytical DAG + KV transfer；所有 `duration_mode` 均可挂载）。
- **`frontier_predictor`**：`predict` 模式的时长真源；应与 preset 指向同一模型族，但 RF 不读取 `ModelConfig` 字段做计时。
...
| 路径 | 角色 |
|------|------|
| `workload_generators/predictors/frontier.py` | ScheduleBatch → Frontier RF |
| `workload_generators/op_workload_generator.py` | features + DAG + analyzer |
| `analytic_model/rf_catalog.py` | 算子名目录 |
| `analytic_model/operators/attention.py` | dense/MLA 代价（含 KV 上下文） |
